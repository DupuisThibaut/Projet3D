...
public:
    ...
    RaySphereIntersection intersect(const Ray &ray) const {
        ...
        if (t1 > 0) {t = t1;intersection.intersection=ray.origin()+t1*ray.direction();
            intersection.normal=Vec3((intersection.intersection-m_center)/m_radius);
        }
        ...
        return intersection; // N'oubliez pas de retourner l'intersection
    }
};
#endif
